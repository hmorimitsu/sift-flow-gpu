...
class Flow(object):
    ...
    def _writeFlow(self, path, flow_array):
        assert type(path) is str, "Input [{p}] is not a string".format(p=path)
        assert path.split('.')[-1] == 'flo', "File extension [flo] required, [{f}] given".format(f=path.split('.')[-1])

        with open(path, 'wb') as flo:
            tag = np.array([self.TAG_FLOAT], np.float32)
            tag.tofile(flo)

            width = np.array([flow_array.shape[1]], np.int32)
            width.tofile(flo)

            height = np.array([flow_array.shape[0]], np.int32)
            height.tofile(flo)

            flow_array.tofile(flo)
